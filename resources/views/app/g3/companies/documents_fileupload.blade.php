...
<h1>{{__('general.Companies')}} - {{$data['company']->name}} - {{__('general.Documents')}} - {{__('general.Upload')}}</h1>

@stop

@section('content')
<a href="{{route('companies.documents', $data['company']->id)}}" class="btn btn-app pull-right">
	<i class="fa fa-arrow-left"></i> {{__('general.Documents')}}
</a>
		...
		<div class="box-body table-responsive">
           ...
           <form id="add_form" action="" method="post" enctype="multipart/form-data">
              ...
              <div class="box-body">
    			...
    			<input type="hidden" name="employee" value="{{$data['company']->id}}">
